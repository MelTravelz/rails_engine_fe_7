class TeaShopFacade
  ...
  def get_all_merchants
    ...
    all_merchants = search_results[:data].map do |merchant|
      merch_hash = { merchant_id: merchant[:id].to_i, merchant_name: merchant[:attributes][:name] } 
  ...
  def get_merch_items(merch_id)
    ...
    items_info[:data].map do |item|
      item_hash = { id: item[:id].to_i, name: item[:attributes][:name], description: item[:attributes][:description], unit_price: item[:attributes][:unit_price] }
      Item.new(item_hash)
    end
  end
